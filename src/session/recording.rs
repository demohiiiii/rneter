...
impl std::fmt::Debug for SessionRecorder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("SessionRecorder")
            ...
            .field("id", &self.id)
            .field("connection_owned", &self.connection_owned)
...
impl SessionRecorder {
    ...
    /// Return the identifier generated by rneter for this recorder.
    ///
    /// Recorder identifiers are read-only and cannot be supplied by callers.
    pub fn id(&self) -> u64 {
        self.id
    }

    pub(crate) fn new_connection_owned(level: SessionRecordLevel) -> Self {
        Self {
    ...
    pub(crate) fn is_connection_owned(&self) -> bool {
        self.connection_owned
    }
